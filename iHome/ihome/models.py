...
class User(BaseModel, db.Model):
    ...
    # def generate_password_hash(self, origin_passwoed):
    #     """对密码加密"""
    #     self.password_hash = generate_password_hash(origin_passwoed)
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
        ...
        # 评论信息
        comments = []
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None).order_by(Order.update_time.desc()).limit(constants.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS).all()
