...
def get_house_index():
    ...
    try: 
        ...
        return '{{"errno":0, "errmsg":"OK", "data":{}}}'.format(json_houses), 200, {"Content-Type": "application/json"}
...
def get_house_detail(house_id):
    ...
    # 检验参数
    if not house_id:
        return jsonify(errno=RET.PARAMERR, errmsg="参数正确")
    ...
    if ret:
        ...
        return '{{"errno": "0", "errmsg":"OK", "data":{{"user_id":{}, "house":{}}}}}'.format(user_id, ret), \
    ...
    try:
        redis_store.setex("house_info_{}".house_id, constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND, json_house)
    except Exception as e:
        current_app.logger.error(e)

    resp = '{{"errno": "0", "errmsg":"OK", "data":{{"user_id":{}, "house": {}}}}}'.format(user_id, json_house), 200, {"Content-Type": "application/json"}
    return resp
...
def get_house_list():
    ...
    # 获取缓存数据
    redis_key = "house_{}_{}_{}_{}".format(start_date, end_date, area_id, sort_key)
    try:
    ...
    # 处理分页
    try:
        # house_query.paginate(当前页数, 当前页数, 自动的错误输出)
    ...
    if page <= total_page:
        ...
        return jsonify(errno=RET.NODATA, errmsg="页面不存在")
